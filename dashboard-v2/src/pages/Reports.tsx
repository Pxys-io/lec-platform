      ...
      <div className="card" style={{ padding: 0 }}>
        <table>
          ...
          <tbody data-testid="reports-table">
            {reports.map((r) => (
              <tr key={r.id}>
                <td><span className="badge b-blue">{r.target_type}</span></td>
                <td><b>{r.reason}</b></td>
                <td className="sub" style={{ margin: 0 }}>{r.description || '—'}</td>
                <td><span className={`badge ${r.status === 'resolved' ? 'b-green' : r.status === 'dismissed' ? 'b-gray' : 'b-yellow'}`}>{r.status}</span></td>
