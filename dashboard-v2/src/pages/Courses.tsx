...
export default function Courses() {
  ...
  const createCourse = async () => {
    try {
      await api.post('/courses', { title: cTitle, description: cDesc, visibility: cVis, tags: [], thumbnail_url: null })
  ...
  const createLesson = async (courseId: string) => {
    if (!lTitle) return
    try {
      await api.post('/lessons', { course_id: courseId, title: lTitle, description: '', order: lessons.length, lock_type: 'none', is_published: false })
            ...
            <div className="card" key={c.id} style={{ padding: 0 }} data-testid={`course-${c.title}`}>
              ...
              {openId === c.id && (
                <div style={{ padding: '0 18px 18px', borderTop: '1px solid var(--border)' }}>
                  <table>
                    <thead><tr><th>#</th><th>Lesson</th><th>Video</th><th>Published</th><th></th></tr></thead>
                    <tbody data-testid="lessons-table">
                      ...
                      {lessons.length === 0 && <tr><td colSpan={5} className="sub" style={{ textAlign: 'center' }}>No lessons yet</td></tr>}
                    </tbody>
                  </table>
                  <div className="row" style={{ marginTop: 12 }}>
                    <input placeholder="New lesson title…" value={lTitle} onChange={(e) => setLTitle(e.target.value)} style={{ maxWidth: 320 }}
