...
from app.models.user import User, UserRole, UserProfile, UserCourseAccess, PanicModeConfig
...
from app.models.interaction import WatchHistory, Message, AccessCode, QBankEnrollment, QBankSession
...
def seed():
    ...
    with Session(engine) as session:
        ...
        qbank = QBank(
            id=_id(),
            title="Medical Licensing Exam Prep",
            description="Comprehensive question bank for medical students.",
            instructor_id=inst_id,
            tags=json.dumps(["Cardiology", "Neurology", "Pharmacology", "Anatomy"]),
            visibility="public",
            price=0.0,
            created_at=_now(),
            updated_at=_now(),
        ...
        q_bank_questions = [
            (
                ...
                json.dumps(["Mitral valve", "Tricuspid valve", "Aortic valve"]),
                "Mitral valve",
            ...
            (
                ...
                json.dumps(["Humerus", "Femur", "Tibia"]),
                "Femur",
                "The femur is the thigh bone and is the longest and strongest bone in the body.",
                ["Anatomy"],
                2,
            ),
        ]
        for qid, qtype, question, opts, answer, exp, tags, order in q_bank_questions:
            session.add(
                Question(
                    id=_id(),
                    qbank_id=qid,
        ...
        # Seed Panic Mode Config (inactive by default)
        session.add(
            PanicModeConfig(
                id=_id(),
                is_active=False,
        ...
        lessons_data = [
            ...
            (
                ...
                "previous_lesson",
                True,
                video_ids[1] if video_ids else None,
            ),
            (
                courses[0].id,
                "Functions",
                ...
                "previous_lesson",
                True,
                video_ids[2] if video_ids else None,
            ),
            (
                courses[0].id,
            ...
            (
                ...
                "none",
                True,
                video_ids[3] if video_ids else None,
            ),
            (
                courses[1].id,
                "FastAPI Backend",
                ...
                "previous_lesson",
                True,
                video_ids[4] if video_ids else None,
            ),
            (
                courses[1].id,
                "Deployment",
                ...
                "previous_lesson",
                True,
                video_ids[5] if video_ids else None,
            ),
            (
                courses[2].id,
                "Statistics Review",
