...
class AccessCode(SQLModel, table=True):
    ...
    course_id: Optional[str] = Field(
        default=None, nullable=True, foreign_key="courses.id"
    )
    lesson_id: Optional[str] = Field(
        default=None, nullable=True, foreign_key="lessons.id"
    )
...
class VideoModePolicy(SQLModel, table=True):
    __tablename__ = "video_mode_policies"

    id: str = Field(default_factory=lambda: str(uuid.uuid4()), primary_key=True)
    video_id: str = Field(unique=True, index=True)
    lesson_id: Optional[str] = Field(default=None, nullable=True)
    mode_when_downloaded: str  # "cloud_only" | "local_only" | "hybrid"
    current_action: str = Field(default="allow")
    auto_deleted: bool = Field(default=False)
