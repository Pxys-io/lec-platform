...
class Quiz(SQLModel, table=True):
    ...
    id: str = Field(default_factory=lambda: str(uuid.uuid4()), primary_key=True)
    lesson_id: str = Field(foreign_key="lessons.id", unique=True, nullable=True)
    title: str
...
class QBank(SQLModel, table=True):
    __tablename__ = "qbanks"

    id: str = Field(default_factory=lambda: str(uuid.uuid4()), primary_key=True)
    title: str
    description: str = ""
    instructor_id: str = Field(foreign_key="users.id")
    tags: str = Field(default="[]") # JSON list of subjects/tags
    visibility: CourseVisibility = Field(default=CourseVisibility.PRIVATE)
    thumbnail_url: Optional[str] = Field(default=None, nullable=True)
...
class Question(SQLModel, table=True):
    ...
    id: str = Field(default_factory=lambda: str(uuid.uuid4()), primary_key=True)
    quiz_id: Optional[str] = Field(default=None, nullable=True, foreign_key="quizzes.id")
    qbank_id: Optional[str] = Field(default=None, nullable=True, foreign_key="qbanks.id")
    type: str = Field(default="multiple_choice")
    question: str
    options: Optional[str] = Field(default=None, nullable=True) # JSON list of options
    correct_answer: str
