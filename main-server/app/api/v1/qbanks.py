...
def get_qbank(
    ...
    return QBankResponse(
        ...
        instructor_id=qbank.instructor_id,
        tags=json.loads(qbank.tags) if qbank.tags else [],
        visibility=qbank.visibility,
        thumbnail_url=qbank.thumbnail_url,
        price=qbank.price,
        created_at=qbank.created_at,
        updated_at=qbank.updated_at,
    )
...
def get_qbank_questions(
    ...
    if not enrollment and user.role not in [
        UserRole.ADMIN,
        UserRole.SUPER_ADMIN,
        UserRole.INSTRUCTOR,
    ]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Not enrolled in this QBank"
        )
...
def create_qbank_session(
    ...
    selected_questions = random.sample(
        filtered_questions, min(len(filtered_questions), request.count)
    )
    ...
    session = QBankSession(
        ...
        config_json=json.dumps(
            {"subjects": request.subjects, "mode": request.mode, "count": request.count}
        ),
