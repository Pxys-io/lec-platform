...
def seed():
    ...
    with Session(engine) as session:
        ...
        video_ids = []
        video_ids_file = Path(__file__).parent.parent.parent / "test_video_ids.json"
        if video_ids_file.exists():
            video_ids = json.loads(video_ids_file.read_text())
            print(f"  Loaded {len(video_ids)} video IDs from test_video_ids.json")
        else:
            video_ids = []
        ...
        lessons_data = [
            (
                ...
                "none",
                True,
                video_ids[0] if len(video_ids) > 0 else None,
            ),
            (
                courses[0].id,
                "Control Flow",
                ...
                "previous_lesson",
                True,
                video_ids[1] if len(video_ids) > 1 else None,
            ),
            (
                courses[0].id,
                "Functions",
                ...
                "previous_lesson",
                True,
                video_ids[2] if len(video_ids) > 2 else None,
            ),
            (
                courses[0].id,
            ...
            (
                ...
                "none",
                True,
                video_ids[3] if len(video_ids) > 3 else None,
            ),
            (
                courses[1].id,
                "FastAPI Backend",
                ...
                "previous_lesson",
                True,
                video_ids[4] if len(video_ids) > 4 else None,
            ),
            (
                courses[1].id,
                "Deployment",
                ...
                "previous_lesson",
                True,
                video_ids[5] if len(video_ids) > 5 else None,
            ),
            (
                courses[2].id,
                "Statistics Review",
