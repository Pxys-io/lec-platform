...
class _VideoPlayerScreenState extends State<VideoPlayerScreen> {
  ...
  Future<void> _loadAndPlay(VideoResolution resolution) async {
    try {
      ...
      final isDownloaded = await downloader.isDownloaded(widget.lessonId, resolution.resolution);
      ...
      if (isDownloaded) {
        await _localServer.start('${appDir.path}/downloads');
        videoUrl = 'http://localhost:${_localServer.port}/playlist/${widget.lessonId}/${resolution.resolution}.m3u8';
        headers = {'Authorization': 'Bearer ${_localServer.authToken}'};
        _isLocal = true;
        
        final playlistFile = File('${appDir.path}/downloads/${widget.lessonId}/${resolution.resolution}/playlist.m3u8');
      ...
      if (_watermarkTimes.isNotEmpty) {
        dev.log('Watermarked segments at: ${_watermarkTimes.map((t) => '${t.toStringAsFixed(1)}s').join(', ')}');
      }
  ...
  Future<void> _startDownload(VideoResolution resolution) async {
    ...
    try {
      ...
      final playlistContent = await videoRepo.getPlaylist(widget.lessonId, resolution.resolution);
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          if (debugMode && _watermarkTimes.isNotEmpty && _videoPlayerController != null)
            Positioned(
              bottom: 55,
              left: 0,
              right: 0,
              ...
              child: LayoutBuilder(
                builder: (ctx, constraints) {
                  final total = _videoPlayerController!.value.duration.inMilliseconds / 1000.0;
