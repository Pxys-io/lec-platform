...
class VideoDownloader {
  ...
  Future<void> downloadVideo({
    ...
    // Save the original playlist (we'll modify it in the server when serving)
    final playlistFile = File('${downloadDir.path}/playlist.m3u8');
    await playlistFile.writeAsString(playlistContent);

    // Parse segments: any line that doesn't start with # and is not empty
    ...
    for (int i = 0; i < segments.length; i++) {
      ...
      if (segmentLine.startsWith('http')) {
        ...
        segmentUrl = '$baseUrl/videos/$lessonId/segments/$resolution/$segmentLine';
      }
      ...
      final encryptedData = EncryptionHelper.encrypt(Uint8List.fromList(response.data!));
    ...
    for (int i = 0; i < segments.length; i++) {
      ...
      newPlaylistContent = newPlaylistContent.replaceFirst(segments[i], 'segment_$i.ts');
    }
