    ...
    return Scaffold(
      ...
      body: BlocBuilder<DownloadsCubit, DownloadsState>(
        builder: (context, state) {
          if (state.isLoading && state.completed.isEmpty && state.active.isEmpty) {
          ...
          return ListView(
            ...
            children: [
              if (state.active.isNotEmpty) ...[
                Text('Currently Downloading', style: Theme.of(context).textTheme.titleMedium),
                const SizedBox(height: 12),
                ...state.active.map((item) => _buildActiveDownload(context, item)),
                const SizedBox(height: 24),
              ],
              if (state.completed.isNotEmpty) ...[
                Text('Downloaded Videos', style: Theme.of(context).textTheme.titleMedium),
                const SizedBox(height: 12),
                ...state.completed.map((item) => _buildCompletedDownload(context, item)),
              ],
            ],
          );
        },
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text('${item.resolution} | ${item.downloadedSegments}/${item.totalSegments} segments', 
                ...
                IconButton(
                  ...
                  onPressed: () => context.read<DownloadsCubit>().cancelDownload(item.lessonId, item.resolution),
                ),
              ],
            ),
  ...
  Widget _buildCompletedDownload(BuildContext context, CompletedDownload item) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text('${item.resolution} | ${sizeMb.toStringAsFixed(1)} MB'),
        trailing: PopupMenuButton(
          itemBuilder: (context) => [
            const PopupMenuItem(value: 'play', child: Text('Play Offline')),
          ...
          onSelected: (val) {
            if (val == 'play') {
              ...
              context.read<DownloadsCubit>().deleteDownload(item.lessonId, item.resolution);
            }
          },
        ),
        onTap: () {
