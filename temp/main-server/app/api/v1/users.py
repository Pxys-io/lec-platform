...
from app.models import User, UserProfile, UserCourseAccess, UserDevice, UserRole, Course, QBankEnrollment, QBank
...
def reset_user_devices(
    ...
    return {"message": f"All devices reset for user {user_id}"}


@router.post("/{user_id}/qbank-access", response_model=dict)
def grant_qbank_access(
    user_id: str,
    request: QBankAccessCreate,
    db: Session = Depends(get_db),
    admin: User = Depends(require_admin),
):
    target_user = db.get(User, user_id)
    ...
    qbank = db.get(QBank, request.qbank_id)
    if not qbank:
        raise HTTPException(
    ...
    existing = db.exec(
        ...
        .where(QBankEnrollment.qbank_id == request.qbank_id)
        .where(QBankEnrollment.user_id == user_id)
    ).first()

    if existing:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="User already enrolled in this QBank",
        )
    ...
    return {"message": "QBank access granted", "enrollment_id": enrollment.id}


@router.delete("/{user_id}/qbank-access/{enrollment_id}")
def revoke_qbank_access(
    user_id: str,
    enrollment_id: str,
    db: Session = Depends(get_db),
    admin: User = Depends(require_admin),
    ...
    return {"message": "QBank access revoked"}
