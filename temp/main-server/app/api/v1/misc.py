...
async def app_handshake(
    ...
    # Check for active panic mode configs
    configs = db.exec(select(PanicModeConfig).where(PanicModeConfig.is_active == True)).all()
    ...
    for config in configs:
        if config.target_type == "global":
            ...
            webview_url = config.webview_url
            break
        elif config.target_type == "platform" and config.target_value == request.platform:
            panic_mode = True
            webview_url = config.webview_url
            break
            ...
            webview_url = config.webview_url
            break
        elif config.target_type == "build" and config.target_value == request.build_number:
            panic_mode = True
            webview_url = config.webview_url
            break
...
def get_overview_stats(
    ...
    if user.role not in [UserRole.ADMIN, UserRole.SUPER_ADMIN, UserRole.INSTRUCTOR]:
        ...
        total_courses = db.exec(select(func.count(UserCourseAccess.id)).where(UserCourseAccess.user_id == user.id)).first() or 0
        total_lessons = db.exec(select(func.count(WatchHistory.id)).where(WatchHistory.user_id == user.id).where(WatchHistory.completion_percentage >= 90)).first() or 0
        total_quizzes = db.exec(select(func.count(QuizAttempt.id)).where(QuizAttempt.user_id == user.id)).first() or 0
        total_watch = db.exec(select(func.sum(WatchHistory.watch_time)).where(WatchHistory.user_id == user.id)).first() or 0
...
async def get_video_manifest_proxy(
    ...
    if not check_lesson_access(db, user, lesson):
        raise HTTPException(status_code=403, detail=f"Access denied (Student {user.id} has no access to lesson {lesson.id})")

    async with httpx.AsyncClient() as client:
        url = f"{settings.VIDEO_SERVER_BASE_URL}/internal/videos/{lesson.video_id}/manifest"
...
async def proxy_video_playlist(
    ...
    async with httpx.AsyncClient() as client:
        url = f"{settings.VIDEO_SERVER_BASE_URL}/internal/videos/{lesson.video_id}/playlist/{resolution}"
        ...
        content = r.text
        correct_base = settings.VIDEO_SERVER_BASE_URL.rstrip("/")
        content = re.sub(
...
async def upload_init(
    ...
    async with httpx.AsyncClient() as client:
        ...
        r = await client.post(
            f"{settings.VIDEO_SERVER_BASE_URL}/internal/videos/upload/init",
            json=payload,
            headers=headers,
            timeout=10.0,
        )
...
async def upload_chunk(
    ...
    async with httpx.AsyncClient() as client:
        ...
        headers = {"Authorization": f"Bearer {settings.VIDEO_SERVER_INTERNAL_TOKEN}"}
        r = await client.post(
            f"{settings.VIDEO_SERVER_BASE_URL}/internal/videos/upload/{upload_id}/chunk",
...
async def upload_status(
    ...
    async with httpx.AsyncClient() as client:
        headers = {"Authorization": f"Bearer {settings.VIDEO_SERVER_INTERNAL_TOKEN}"}
        r = await client.get(
            f"{settings.VIDEO_SERVER_BASE_URL}/internal/videos/upload/{upload_id}/status",
            headers=headers,
            timeout=10.0,
        )
        if r.status_code != 200:
...
async def upload_complete(
    ...
    async with httpx.AsyncClient() as client:
        headers = {"Authorization": f"Bearer {settings.VIDEO_SERVER_INTERNAL_TOKEN}"}
        r = await client.post(
            f"{settings.VIDEO_SERVER_BASE_URL}/internal/videos/upload/{upload_id}/complete",
            headers=headers,
            timeout=60.0,
        )
        if r.status_code != 200:
...
async def list_manage_videos(
    ...
    async with httpx.AsyncClient() as client:
        ...
        r = await client.get(
            f"{settings.VIDEO_SERVER_BASE_URL}/internal/videos",
    ...
    if user.role == UserRole.INSTRUCTOR:
        ...
        if not lesson:
            async with httpx.AsyncClient() as client:
                headers = {"Authorization": f"Bearer {settings.VIDEO_SERVER_INTERNAL_TOKEN}"}
                r = await client.get(
                    f"{settings.VIDEO_SERVER_BASE_URL}/internal/videos/{video_id}",
                    headers=headers,
                )
                if r.status_code == 200:
                    video_data = r.json()
                    ...
                    if str(owner_id) != str(user.id):
                        raise HTTPException(status_code=403, detail=f"Access denied (Video owner: {owner_id}, You: {user.id})")
                else:
                    raise HTTPException(status_code=r.status_code, detail="Video not found on video server")
...
async def list_manage_jobs(
    ...
    async with httpx.AsyncClient() as client:
        ...
        r = await client.get(
            f"{settings.VIDEO_SERVER_BASE_URL}/internal/videos/jobs",
            params=params,
            headers=headers
        )
        if r.status_code != 200:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if jr.status_code == 200:
                ...
                if vr.status_code == 200:
                    ...
                    raise HTTPException(status_code=vr.status_code, detail="Video not found on video server")
            else:
                raise HTTPException(status_code=500, detail="Failed to fetch jobs from video server")

    async with httpx.AsyncClient() as client:
        headers = {"Authorization": f"Bearer {settings.VIDEO_SERVER_INTERNAL_TOKEN}"}
        r = await client.post(
            f"{settings.VIDEO_SERVER_BASE_URL}/internal/videos/jobs/{job_id}/kill",
            headers=headers
        )
        if r.status_code != 200:
...
async def transcode_manage_video(
    ...
    async with httpx.AsyncClient() as client:
        headers = {"Authorization": f"Bearer {settings.VIDEO_SERVER_INTERNAL_TOKEN}"}
        r = await client.post(
            f"{settings.VIDEO_SERVER_BASE_URL}/internal/videos/{video_id}/transcode",
...
async def get_manage_video_manifest(
    ...
    async with httpx.AsyncClient() as client:
        headers = {"Authorization": f"Bearer {settings.VIDEO_SERVER_INTERNAL_TOKEN}"}
        url = f"{settings.VIDEO_SERVER_BASE_URL}/internal/videos/{video_id}/manifest"
    ...
    async def stream_video():
        async with httpx.AsyncClient() as client:
            ...
            async with client.stream(
                "GET",
                f"{settings.VIDEO_SERVER_BASE_URL}/internal/videos/{video_id}/raw",
                headers=headers,
                timeout=None
            ) as r:
                if r.status_code != 200:
...
async def get_manage_video_playlist(
    ...
    async with httpx.AsyncClient() as client:
        headers = {"Authorization": f"Bearer {settings.VIDEO_SERVER_INTERNAL_TOKEN}"}
        url = f"{settings.VIDEO_SERVER_BASE_URL}/internal/videos/{video_id}/playlist/{resolution}"
    ...
    content = r.text
    correct_base = settings.VIDEO_SERVER_BASE_URL.rstrip("/")
    # Replace any hardcoded localhost URLs with the configured base URL
    content = re.sub(
        r"https?://(?:localhost|127\.0\.0\.1):8001(?:/api/v1)?/",
        correct_base + "/",
        content,
