...
class QuizRepository {
  ...
  // QBanks
  Future<List<QBank>> getQBanks() async {
    final response = await apiClient.get('/qbanks');
    return (response as List)
        .map((e) => QBank.fromJson(Map<String, dynamic>.from(e as Map)))
        .toList();
  }

  Future<QBank> getQBank(String id) async {
    final response = await apiClient.get('/qbanks/$id');
    return QBank.fromJson(Map<String, dynamic>.from(response as Map));
  }

  Future<QBankEnrollment> enrollQBank(String qbankId, Map<String, dynamic> formData) async {
    final response = await apiClient.post('/qbanks/$qbankId/enroll', body: {
      'form_data': formData,
    });
    return QBankEnrollment.fromJson(Map<String, dynamic>.from(response as Map));
  }

  Future<List<Question>> getQBankQuestions(String qbankId) async {
    final response = await apiClient.get('/qbanks/$qbankId/questions');
    return (response as List)
        .map((e) => Question.fromJson(Map<String, dynamic>.from(e as Map)))
        .toList();
  }

  Future<QBankSession> createQBankSession(String qbankId, Map<String, dynamic> data) async {
    final response = await apiClient.post('/qbanks/$qbankId/sessions', body: data);
    return QBankSession.fromJson(Map<String, dynamic>.from(response as Map));
  }

  Future<List<QBankSession>> getRecentSessions() async {
    final response = await apiClient.get('/qbanks/sessions/recent');
    return (response as List)
